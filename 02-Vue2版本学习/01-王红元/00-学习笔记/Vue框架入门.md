...
> **组件其实就是实例**。
>
> 一个Vue项目是由一个**根实例和多个组件**组成，组件其实就是实例。只不过是为了区分使用方式，所以才用了两个叫法，即组件和实例
>
> 区分什么使用方式呢？
>
...
>
> 现在是2021年3月21日，我再来看一遍，希望这次彻底掌握它了啊。
...
Webpack就是一个**前端模块化打包工具**，之后你可以向面试官解释一下**模块化**的概念，再向面试官解释一下**打包**的概念。
...
> 你用ES6写的代码，要转成ES5；你用Sass,Less写的代码，要转成CSS；你用TypeScript写的代码，要转成JavaScript。 **我们不能直接给它放到服务器，总要经过一些工具，进行一下打包和转化**，生成**浏览器可以识别并且执行**的代码。
...
> Node版本不能太低，以后还要用**脚手架**，对Node版本要求大于V8.9。
...
[vue-cli是什么？和 webpack是什么关系？](https://www.zhihu.com/question/56615554)

[使用vue-cli，还有必要学习webpack吗？](https://www.zhihu.com/question/65510789)
...
> 早期在浏览器中输入**域名**，服务器会直接生成HTML+CSS+一些Java代码，然后Java代码会从数据库读取数据，然后**直接传给浏览器**，你看到的页面是**服务器直接渲染**而来的，这个就叫后端渲染(通过特殊的技术，如JSP等)。
>
...
> **单页面富应用阶段**(SPA)在**前后端分离**的基础上，又添加了一层**前端路由**的概念。

![](Vue框架入门/190.png)

> 你写的**所有代码**，都是放到**静态资源服务器**里面的。
>
> 你在浏览器输入url,静态资源服务器会给你返回相应的`HTML+CSS+JS`，其中`HTML`和`CSS`浏览器可以直接渲染，而`JS`要**由浏览器先执行**。
>
> 当浏览器执行JS代码的时候，发现你有API请求，这个时候它才会去**API服务器**进行相应的请求。
>
> **网页端和手机端**淘宝请求的都是同一个服务器，前后端分离的话，服务器就不用管你到底是通过电脑请求的，还是通过手机来请求的了，只要提供对应的API接口就可以了。
...
**index.js**(位置在src文件夹下的**router文件夹**里面)
...
> 现在是` import Router from "vue-router";`和`const router = new Router({})`了。前面没有Vue了。
...
export default {
  ...
  // 我对App.vue也是真的不熟！！！
  // 我们在这里导出了App.vue,在main.js里面导入了App.vue
...
>
> 现在是2021年5月11日，面试中老师问道Vuex，我来复习一遍吧，几乎忘光了。
>
> 现在是2021年8月20日，我又一次学习了Vuex，只不过这次看的是张天禹的Vue课程，我有一个体会，一个知识点看起来很难理解，有时候不是自己智力有问题，而是老师讲的有问题，王红元和张天禹比起来他讲的是真的乱啊，对比起来张天禹讲的是真的好！！！
...
> **变量**一般是用来保存我们的状态的，而我们的变量一般保存到data里面。
