 ...
 * @LastEditTime: 2021-08-16 10:17:22
...
export default {
  ...
  data() {
    return {
      ...
      //刚开始是没有数据的，是空的，初始化的时候从localStorage中去拿
      //   ],
      //1   todos: JSON.parse(localStorage.getItem("todos")),
      todos: JSON.parse(localStorage.getItem("todos")) || [],
      //   初始的todos不是空数组，而是一个null,所以后面要写一个[]，否则会报错
    };
  },
  methods: {
    //添加一个todo
    addTodo(todoObj) {
      ...
      //unshift是数组头部添加
      //unshift添加Vue是可以捕获到的，是可以watch到的
    },
    //勾选或者取消勾选一个todo
    checkTodo(id) {
  ...
  watch: {
    ...
    //   //   这样添加不行，因为每一个todo项是一个对象，而不是一个单纯的字符串，是一个名字
    //   //   一个Bug:当你点击复选框时，watch是监视不到的，它只能监视todos这个对象，里面的属性监视不到，所以我们要写完整版的todos
    // },
    todos: {
      deep: true,
      //必须要开启深度监视
